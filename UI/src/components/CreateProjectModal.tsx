    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-[640px] rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="border-b border-slate-100 p-6 flex items-center justify-between">
          <div>
            <h2 className="text-[#0d151b] text-2xl font-bold tracking-tight">Create New Project</h2>
            <p className="text-slate-500 text-sm mt-1 font-display">Set up your workspace and invite your team.</p>
          </div>
          <button 
            onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex flex-col gap-2">
              <label className="flex flex-col w-full">
                <p className="text-[#0d151b] text-sm font-semibold leading-normal pb-1">Project Name</p>
                <input 
                  name="name"
                  value={formData.name}
                  ...
                  className={`form-input flex w-full rounded-lg text-[#0d151b] focus:outline-0 focus:ring-2 focus:ring-blue-600/20 border h-12 placeholder:text-[#4c759a] px-4 text-base font-normal ${
                    errors.name ? 'border-red-300 bg-red-50' : 'border-[#cfdce7] bg-white focus:border-blue-600'
                  }`}
            ...
            <div className="flex flex-col gap-2">
              <label className="flex flex-col w-full">
                <p className="text-[#0d151b] text-sm font-semibold leading-normal pb-1">Description (Optional)</p>
                <textarea 
                  name="description"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                <label className="flex flex-col w-full">
                  <p className="text-[#0d151b] text-sm font-semibold leading-normal pb-1">Start Date (Optional)</p>
                  <input 
                    name="startDate"
                    type="date"
              ...
              <div className="flex flex-col gap-2">
                <label className="flex flex-col w-full">
                  <p className="text-[#0d151b] text-sm font-semibold leading-normal pb-1">End Date (Optional)</p>
                  <input 
                    name="endDate"
                    type="date"
            ...
            <div className="space-y-3">
              <h3 className="text-[#0d151b] text-sm font-semibold leading-tight tracking-tight">Project Category Color</h3>
            ...
            {/* Team Members */}
            <div className="flex flex-col gap-2">
              <p className="text-[#0d151b] text-sm font-semibold leading-normal pb-1">Add Team Members</p>
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-10 max-h-40 overflow-y-auto">
                    {loadingUsers ? (
                      ...
                      filteredMembers.map(member => (
                        ...
                        >
                          {member.avatar_url ? (
                            <img src={member.avatar_url} alt={member.full_name} className="w-6 h-6 rounded-full" />
                          ) : (
                ...
                {selectedMembers.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {selectedMembers.map(member => (
                      <div key={member.id} className="flex items-center gap-1.5 bg-blue-600/10 text-blue-600 px-3 py-1.5 rounded-full text-xs font-semibold">
                        {member.avatar_url ? (
