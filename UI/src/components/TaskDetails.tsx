...
export default function TaskDetails() {
  ...
  const handleStatusUpdate = async (newStatus: 'To Do' | 'In Progress' | 'Done') => {
    if (!task) return
    try {
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      ...
      case 'Medium': return { bg: 'bg-amber-500', text: 'text-amber-600' }
      case 'Low': return { bg: 'bg-emerald-500', text: 'text-emerald-600' }
          ...
          <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-white">
            <div className="flex items-center gap-4">
              <div className="flex flex-wrap gap-2 text-sm">
                <a className="text-slate-500 font-medium hover:text-blue-600" href="#">Projects</a>
                <span className="text-slate-500 font-medium">/</span>
                <a className="text-slate-500 font-medium hover:text-blue-600" href="#">{task.project.name}</a>
                <span className="text-slate-500 font-medium">/</span>
                <span className="text-slate-900 font-medium">TASK-{task.id.slice(-3)}</span>
              </div>
            </div>
            ...
            <div className="flex-1 p-8 space-y-8 border-r border-slate-200">
              ...
              <div className="space-y-4">
                <div className="flex flex-wrap justify-between items-start gap-4">
                  <h1 className="text-3xl md:text-4xl font-black leading-tight tracking-tight text-slate-900">{task.title}</h1>
                  <div className="flex items-center gap-3">
                    <select 
                      value={task.status}
                      onChange={(e) => handleStatusUpdate(e.target.value as 'To Do' | 'In Progress' | 'Done')}
                    ...
                    >
                      ...
                      <span className="truncate">{task.status === 'Done' ? 'Completed' : 'Mark as Complete'}</span>
                    </button>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <span className={`px-3 py-1 text-xs font-bold rounded uppercase tracking-wider ${getStatusColor(task.status)}`}>
                    {task.status}
                  </span>
                  <p className="text-slate-500 text-sm">
                    Created by {task.creator.full_name} • {new Date(task.created_at).toLocaleDateString()}
                  </p>
                </div>
              </div>
              ...
              <div className="border-b border-slate-200">
                <div className="flex gap-8">
                  <a className="flex items-center gap-2 border-b-[3px] border-b-blue-600 text-blue-600 pb-[13px] pt-4 font-bold text-sm" href="#">
                    <span className="material-symbols-outlined text-lg">description</span>
                    <span>Overview</span>
                  </a>
                  <a className="flex items-center gap-2 border-b-[3px] border-b-transparent text-slate-500 pb-[13px] pt-4 font-bold text-sm hover:text-blue-600 transition-colors" href="#">
            ...
            <div className="w-full md:w-80 p-6 bg-slate-50 flex flex-col gap-8">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-slate-500 text-xs font-bold uppercase tracking-widest">Assignee</label>
                ...
                {task.due_date && (
                  <div className="space-y-2">
                    <label className="text-slate-500 text-xs font-bold uppercase tracking-widest">Due Date</label>
                ...
                <div className="space-y-2">
                  <label className="text-slate-500 text-xs font-bold uppercase tracking-widest">Priority</label>
                  <div className="flex items-center gap-3 p-2 hover:bg-white rounded-lg cursor-pointer transition-colors">
                    <div className={`w-3 h-3 rounded-full ${priorityColors.bg}`}></div>
                    <span className={`text-sm font-semibold ${priorityColors.text}`}>{task.priority}</span>
                  </div>
                </div>
              </div>
