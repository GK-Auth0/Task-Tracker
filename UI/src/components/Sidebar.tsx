...
export default function Sidebar() {
  ...
  return (
    <aside className={`${sidebarCollapsed ? 'w-20' : 'w-72'} flex-shrink-0 border-r border-slate-200 bg-white flex flex-col justify-between transition-all duration-300 ${sidebarCollapsed ? 'p-3' : 'p-6'}`}>
        ...
        <nav className="flex flex-col gap-1">
          ...
          >
            <span
              ...
              style={{ fontVariationSettings: isActive('/projects') ? "'FILL' 1" : "'FILL' 0" }}
            >
              folder_open
            </span>
            {!sidebarCollapsed && <p className={`text-sm ${isActive('/projects') ? 'font-semibold' : 'font-medium'}`}>Projects</p>}
          </Link>
          <Link
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${sidebarCollapsed ? 'justify-center' : ''} ${
              isActive('/dashboard') 
          ...
          >
            ...
            {!sidebarCollapsed && <p className={`text-sm ${isActive('/dashboard') ? 'font-semibold' : 'font-medium'}`}>Tasks</p>}
          </Link>
          <Link
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${sidebarCollapsed ? 'justify-center' : ''} ${
              isActive('/calendar') 
          ...
          >
            <span
              className="material-symbols-outlined"
              style={{ fontVariationSettings: isActive('/team') ? "'FILL' 1" : "'FILL' 0" }}
            >
              group
            </span>
            {!sidebarCollapsed && <p className={`text-sm ${isActive('/team') ? 'font-semibold' : 'font-medium'}`}>Team</p>}
          </Link>
          <Link
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${sidebarCollapsed ? 'justify-center' : ''} ${
          ...
          >
            ...
            {!sidebarCollapsed && <p className="text-sm font-medium">Settings</p>}
          </Link>
        </nav>
      </div>
